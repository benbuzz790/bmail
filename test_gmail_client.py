...
class TestGmailClient(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.test_body = (
            'This is a test email from the Gmail client integration tests.')

    def test_1_send_gmail(self):
        """Test sending an email."""
        print('\nSending test email...')
        result = gmail_client.send_gmail(self.service, self.test_email, '',
    ...
    def test_2_list_emails(self):
        ...
        self.assertTrue(self.test_subject in result,
            f"Test subject '{self.test_subject}' not found in result")
    ...
    def test_3_get_email(self):
        ...
        messages = self.service.users().messages().list(userId='me', q=
            f'subject:{self.test_subject}').execute().get('messages', [])
